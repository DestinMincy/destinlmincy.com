...
- **minor — Functional Correctness**: membership attach/remove revalidated only the relationship detail page, leaving the active-member count on the relationships list stale. **Resolved:** both actions also revalidate `/admin/relationships`.
- **minor — Functional Correctness**: lifecycle database-error states dropped the validated submitted lifecycle, while an error without one could pass an invalid value into the typed select state. **Resolved:** database errors return the validated lifecycle; `LifecycleControl` falls back to `current` when the error state has none.
- **minor — Functional Correctness**: the uncontrolled attach email input retained its value after a successful attach reset action state to idle. **Resolved:** the input is keyed by action state, preserving failures while remounting empty after success.
