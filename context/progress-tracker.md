...
- Made project create/update writes serializable so an application/site cannot be archived between selection validation and persistence; added a concurrent archive/save integration regression test. Typecheck, unit tests, PostgreSQL integration, production build, and the uncommitted CodeRabbit review all pass. Committed-review findings were resolved by requiring `DATABASE_URL` in the Unit 05 E2E fixture guard, enforcing the create-time new-asset/attachment invariant inside the write boundary with an integration regression test, and typing project application options from the shared generated status contract.
